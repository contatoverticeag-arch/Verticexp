...
const strategicCurrencyReference = () => ({
  source: "Referência estratégica",
  usd: { brl: 5.70, pct: 0, reference: true },
  eur: { brl: 6.15, pct: 0, reference: true }
});
...
exports.handler = async () => {
  ...
  try {
    ...
    const fx = strategicCurrencyReference();
    payload.sources.currencies = fx.source;
    payload.currencies.usd = fx.usd;
    payload.currencies.eur = fx.eur;
    payload.errors.push({ source: "cambio", message: "Fontes ao vivo indisponiveis; usando referencia estrategica." });
  }
